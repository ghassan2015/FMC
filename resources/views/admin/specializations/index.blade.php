...
@section('title', __('label.specializations'))
@section('toolbarSubTitle', __('label.specialization_list'))
@section('toolbarPage', __('label.display_all_specializations'))
...
@section('content')
    <div class="d-flex flex-column flex-md-row flex-stack mb-5 gap-3">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-end gap-2" data-kt-docs-table-toolbar="base">
            ...
            <button type="button" class="btn btn-primary add_specialization" data-bs-toggle="tooltip"
                title="{{ __('label.add_new_specialization') }}">
                <i class="ki-duotone ki-plus fs-2"></i>
                {{ __('label.add_new_specialization') }}
            </button>
            @endcan
        </div>
        <!-- Group actions -->
    ...
    <table id="kt_datatable_example_1" class="table align-middle table-row-dashed fs-6 gy-5 data-table">
        <thead>
            <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                ...
                @if(auth('admin')->user()->can('update_status_specialization'))
    ...
    @include('admin.specializations.Modal.add_edit')
@endsection

@push('scripts')
    @include('admin.specializations.js.index')
@endpush
