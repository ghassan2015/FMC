    ...
    <script>
        ...
        $(document).on('click', '.submit_delete', function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.specializations.delete') }}',
                method: 'POST',
                data: {
                    "id": ids,
                    "_token": "{{ csrf_token() }}",
        ...
        table = $('.data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.specializations.getIndex') }}",
                type: 'get',
                data: function(d) {
        ...
        $(document).on('change', '.check_status', function(event) {
            ...
            $.ajax({
                url: '{{ route('admin.specializations.updateStatus') }}',
                method: 'POST',
                data: {
                    "specialization_id": ids,
                    "is_active": is_active,
                    "_token": "{{ csrf_token() }}",
                },
                success: function(response) {
        ...
        $(document).on('click', '.add_specialization', function() {
            $('#exampleModal').modal('show');

            let _url = "{{ route('admin.specializations.store') }}";
        ...
        $(document).on('click', '.edit', function() {
            let button = $(this);
            // ملء الحقول
            $('#specialization_id').val(button.data('specialization_id'));
            ...
            $('#my-form').attr('action', "{{ route('admin.specializations.update') }}");
            // فتح المودال
            $('#exampleModal').modal('show');
        });
