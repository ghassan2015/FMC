...
@section('title', __('label.articales'))
@section('toolbarSubTitle', __('label.articale_list'))
@section('toolbarPage', __('label.display_all_articales'))
...
@section('content')
    <div class="d-flex flex-column flex-md-row flex-stack mb-5 gap-3">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-end gap-2" data-kt-docs-table-toolbar="base">
            ...
            @can('add_articale')
            <button type="button" class="btn btn-primary add_articale" data-bs-toggle="tooltip"
                title="{{ __('label.add_new_articale') }}">
                <i class="ki-duotone ki-plus fs-2"></i>
                {{ __('label.add_new_articale') }}
            </button>
            @endcan
        </div>
        <!-- Group actions -->
    ...
    <table id="kt_datatable_example_1" class="table align-middle table-row-dashed fs-6 gy-5 data-table">
        <thead>
            <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                ...
                <th style="width: 20%">{{ __('label.title') }}</th>
                <th style="width: 20%">{{ __('label.specializations') }}</th>
    ...
    @include('admin.articales.Modal.add_edit')
@endsection

@push('scripts')
    @include('admin.articales.js.index')
@endpush
