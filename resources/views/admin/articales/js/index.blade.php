    ...
    <script>
        ...
        $(document).on('click', '.submit_delete', function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.articales.delete') }}',
                method: 'POST',
                data: {
                    "id": ids,
                    "_token": "{{ csrf_token() }}",
        ...
        table = $('.data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.articales.getIndex') }}",
                type: 'get',
                data: function(d) {
            ...
            columns: [
                ...
                {
                    data: 'specializations.name.ar', // اسم العمود من السيرفر
                    name: 'specializations', // اسم الحقل لعمليات البحث والفرز
                    defaultContent: '-', // القيمة الافتراضية إذا كان فارغ
                    orderable: false,
                    searchable: true
                }
,
        ...
        $(document).on('change', '.check_status', function(event) {
            ...
            $.ajax({
                url: '{{ route('admin.articales.updateStatus') }}',
                method: 'POST',
                data: {
                    "articale_id": ids,
                    "is_active": is_active,
        ...
        $(document).on('click', '.add_articale', function() {
            $('#exampleModal').modal('show');

            let _url = "{{ route('admin.articales.store') }}";
        ...
        $(document).on('click', '.edit', function() {
            let button = $(this);
            // ملء الحقول
            $('#articale_id').val(button.data('articale_id'));
            $('#title_ar').val(button.data('title_ar'));
            $('#title_en').val(button.data('title_en'));
            $('#description_ar').text(button.data('description_ar'));
            $('#description_en').text(button.data('description_en'));
            ...
            $('#my-form').attr('action', "{{ route('admin.articales.update') }}");
            // فتح المودال
            $('#exampleModal').modal('show');
        });


        var quill_ar = new Quill('#description_ar', {
            modules: {
                toolbar: [
                    [{
                        header: [1, 2, false]
        ...
        $.validator.addMethod("quillRequiredAr", function(value, element) {
            return quill_ar.getText().trim().length > 0;
        }, "الوصف بالعربية مطلوب");

        $.validator.addMethod("quillRequiredEn", function(value, element) {
            return quill_en.getText().trim().length > 0;
        ...
        $("#my-form").validate({
            ...
            rules: {
                ...
                description_ar: {
                    quillRequiredAr: true
                },
                description_en: {
                    quillRequiredEn: true
                }
            ...
            submitHandler: function(form) {
                ...
                data.append('description_ar', quill_ar.root.innerHTML);
                data.append('description_en', quill_en.root.innerHTML);
